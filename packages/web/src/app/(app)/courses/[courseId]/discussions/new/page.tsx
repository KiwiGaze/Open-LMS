...
export default function NewDiscussionTopicPage({ params }: { params: Promise<Params> }) {
  ...
  const mutation = useCreateDiscussionTopic(tenantId, courseId);
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    try {
      const topic = await mutation.mutateAsync({
      ...
      publish({ tone: 'success', title: 'Topic created', description: topic.title });
      router.push(`/courses/${courseId}/discussions/${topic.id}`);
    } catch (error) {
      const message = error instanceof ApiHttpError ? error.message : 'Something went wrong.';
      publish({ tone: 'danger', title: 'Could not create topic', description: message });
    }
  });
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        description="Create a prompt for learners to discuss. Visibility, grading, and rubric attachment are configurable."
        actions={
      ...
      <form className="flex flex-col gap-6" onSubmit={onSubmit} noValidate>
        <Card>
          <CardHeader>
            ...
            <CardDescription>The minimum to publish a topic.</CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col gap-4">
            ...
            >
              <Input
                id="title"
                placeholder="Defending a thesis: pick one argument and respond"
                maxLength={180}
            ...
            <FormField
              id="prompt"
              label="Prompt"
              description="Optional. Plain text or Markdown. Shown to learners on the topic page."
            >
              <Textarea
                id="prompt"
                rows={6}
                maxLength={4000}
                placeholder="Set the question or scenario you want learners to engage with…"
            ...
            {gradingEnabled ? (
              <>
                <FormField
                  ...
                  required
                  description="The maximum score available for a learner's participation."
        ...
        <div className="flex flex-col-reverse items-stretch gap-2 sm:flex-row sm:items-center sm:justify-end">
          <Button asChild intent="ghost">
            <Link href={`/courses/${courseId}/discussions`}>Cancel</Link>
          </Button>
